...
export default function Desktop({ registry, windows, setWindows, onOpenWindow }) {
  const apps = [{ id: 'clonesys', title: 'CloneSys', icon: 'https://cdn-icons-png.flaticon.com/512/2550/2550224.png' }, { id: 'explorer', title: 'Explorer', icon: 'https://cdn-icons-png.flaticon.com/512/3767/3767084.png' }];
  return (
    <div className="h-full w-full relative overflow-hidden bg-cover bg-center" style={{ backgroundImage: `url(${registry?.wallpaper})` }}>
      <div className="p-10 grid grid-cols-4 gap-8">
        {apps.map(app => (
          <button key={app.id} onClick={() => onOpenWindow(app.id, app.title, app.icon)} className="flex flex-col items-center gap-2 group">
            <div className="w-16 h-16 bg-black/40 backdrop-blur-xl rounded-2xl flex items-center justify-center border border-white/10 group-hover:scale-110 transition-all shadow-2xl">
