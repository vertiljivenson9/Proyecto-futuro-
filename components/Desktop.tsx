...
export default function Desktop({ registry, windows, setWindows, onOpenWindow }) {
  ...
  return (
    <div className="h-full w-full relative bg-cover bg-center" style={{ backgroundImage: `url(${registry?.wallpaper})` }}>
      <div className="p-10 grid grid-cols-4 sm:grid-cols-8 gap-8">
        {apps.map(app => (
      ...
      {windows.map(win => <Window key={win.id} win={win} onClose={() => setWindows(prev => prev.filter(w => w.id !== win.id))} onOpenWindow={onOpenWindow} />)}
    </div>
  );
}