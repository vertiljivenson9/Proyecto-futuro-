...
export default function Window({ win, onClose, onOpenWindow }) {
  return (
    <div className="absolute glass rounded-xl border border-white/10 shadow-2xl flex flex-col overflow-hidden" style={{ top: win.y, left: win.x, width: win.width, height: win.height, zIndex: win.zIndex }}>
      ...
      <div className="flex-grow bg-black/60 overflow-auto">
        <AppLauncher appId={win.appId} onOpenWindow={onOpenWindow} onClose={onClose} />
      </div>
    </div>
  );
}