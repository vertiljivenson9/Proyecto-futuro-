...
export const saveFile = async (path, content, ns) => {
  if (!db) return;
  db.transaction(STORE_NAME, 'readwrite').objectStore(STORE_NAME).put({ id: crypto.randomUUID(), name: path.split('/').pop(), type: 'file', path, content, size: content.length, namespace: ns });
};